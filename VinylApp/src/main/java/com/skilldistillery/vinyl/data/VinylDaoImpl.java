...
public class VinylDaoImpl implements VinylDAO {
	...
	public List<Album> searchTitle(String keyword) {
		String jpql = "SELECT DISTINCT a.title FROM Album a WHERE a.title LIKE :keyword ORDER BY a.title ASC";
	...
	public List<String> searchArtist(String keyword) {
		String jpql = "SELECT DISTINCT a.artist FROM Album a WHERE a.artist LIKE :keyword ORDER BY a.artist ASC";
		List<String> artists = em.createQuery(jpql, String.class).setParameter("keyword", "%"+keyword+"%").getResultList(); //TODO return string of only artist name
	...
	@Override
	public List<String> retrieveAllStyles(){
		String jpql = "SELECT DISTINCT a.style FROM Album a ORDER BY a.style ASC";
		List<String> genres = em.createQuery(jpql, String.class).getResultList();
		return genres;
	}
	...
	@Override
	public List<Album> retrieveStyleCatalog(String style) {
		String jpql = "SELECT a FROM Album a WHERE a.style = :style ORDER BY a.title ASC";
		List<Album> catalog = em.createQuery(jpql, Album.class).setParameter("style", style).getResultList();
		return catalog;
	}
}
